...
const PARALEGAL_RELIABILITY_PROJECTION = [
  ...
  "metadata.reliability.role",
  "metadata.reliability.evidenceStatus",
  "metadata.reliability.capabilityIds",
  "metadata.reliability.validationRetries",
  "metadata.reliability.validationFailures",
  "metadata.reliability.validationExhausted",
  "metadata.reliability.retryOutcome",
  "metadata.reliability.reliabilityGap",
  "metadata.reliability.repeatedQuestion",
  "metadata.reliability.questionFamilyKey",
  "metadata.reliability.unknownQuestionCluster",
  "metadata.telemetry.role",
...
function parseParalegalReliabilityOptions(argv = process.argv.slice(2)) {
  const requestedDays = Number(
    argv.find((argument) => /^--days=\d+$/.test(argument))?.split("=")[1] || 30
  );
  const value = (name) =>
    String(
  ...
  return {
    days: Math.max(1, Math.min(requestedDays, PARALEGAL_SUPPORT_TELEMETRY_RETENTION_DAYS)),
    synthetic: argv.includes("--synthetic"),
    stage: value("stage").toLowerCase(),
    since: value("since"),
    completedStageIds: value("completed-stages")
      .split(",")
      .map((entry) => entry.trim().toLowerCase())
      .filter(Boolean),
    openIncidentCount:
      openIncidents &&
      Number.isFinite(Number(openIncidents)) &&
      Number(openIncidents) >= 0
...
async function main() {
  const options = parseParalegalReliabilityOptions();
  ...
  const messages = options.synthetic
    ? buildSyntheticParalegalReliabilityMessages()
    : await loadParalegalReliabilityMessages(options);
  const report = summarizeParalegalReliability(messages, {
    windowDays: options.days,
    operationalMode: getParalegalSupportOperationalMode(process.env),
  });
  ...
  const output = {
    ...
    projection: options.synthetic ? "synthetic_fixture" : PARALEGAL_RELIABILITY_PROJECTION,
    observation: options.stage
      ? { stageId: options.stage, startedAt: options.since }
      : null,
    stageGate,
  };
  console.log(JSON.stringify(output, null, 2));
  if (options.enforceStageGate && stageGate?.passed !== true) {
    process.exitCode = 1;
  }
  return output;
}
...
module.exports = {
  ...
  loadParalegalReliabilityMessages,
  main,
